...
try {
    ...
    if ($response.success) {
        ...
        if ($response.data.teachers_activity.Count -eq 0) {
            ...
            foreach ($teacher in $response.data.teachers_activity[0..4]) {  # عرض أول 5 معلمين فقط
                ...
                Write-Host "      ✅ التحضير: $(if($activity.attendance_recorded) { 'تم (' + $activity.attendance_percentage + '%)' } else { 'لم يتم' })"
                Write-Host "      🎤 التسميع: $(if($activity.recitation_recorded) { 'تم (' + $activity.recitation_percentage + '%)' } else { 'لم يتم' })"
...
try {
    ...
    if ($response.success) {
        ...
        if ($response.data.teachers_statistics.Count -eq 0) {
            ...
            foreach ($teacher in $response.data.teachers_statistics[0..2]) {  # عرض أول 3 معلمين فقط
