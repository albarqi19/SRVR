...
class TeacherController extends Controller
{
    ...
    {        try {
            ...
            $teachers->getCollection()->transform(function ($teacher) {
                ...
                if ($teacher->quranCircle) {
                    ...
                    'نشط' => $teacher->is_active_user ? 'نعم' : 'لا',
    ...
    public function show($id): JsonResponse
    {
        try {
            ...
            $data = [
                'معلومات_أساسية' => [
                    'id' => $teacher->id,
                    'الاسم' => $teacher->name ?? 'غير محدد',                    'البريد_الإلكتروني' => $teacher->identity_number ?? 'غير محدد',
                    'رقم_الهاتف' => $teacher->phone ?? 'غير محدد',
                    'رقم_الهوية' => $teacher->id_number,
                    'المسجد' => [
    ...
    public function teacherStats($id): JsonResponse
    {
        try {
            ...
            // إحصائيات التسميع
            $recitationStats = RecitationSession::whereHas('student.circles', function ($query) use ($teacher) {
            ...
            return response()->json([
                ...
                'الإحصائيات' => [
                    'الحلقات_والطلاب' => [
                        ...
                        'إجمالي_الطلاب' => $totalStudents,
                        'الحلقات_النشطة' => $circles->where('is_active', true)->count(),
                        'متوسط_الطلاب_لكل_حلقة' => $circles->count() > 0 ? round($totalStudents / $circles->count(), 2) : 0
                    ],
            ...
            $attendances->getCollection()->transform(function ($attendance) {
                return [
                    ...
                    'مدة_الحضور' => $attendance->arrival_time && $attendance->departure_time 
                        ...
                        : 'غير محدد',                    'حاضر' => $attendance->is_present ? 'نعم' : 'لا',
                ...
                'البيانات_المالية' => [
                    'الرواتب' => $salaries->map(function ($salary) {
                        return [
                            ...
                            'البدلات' => $salary->allowances,
                            'الخصومات' => $salary->deductions,
                            'صافي_الراتب' => $salary->net_salary,                            'مدفوع' => $salary->is_paid ? 'نعم' : 'لا',
    ...
    private function buildMosqueData($teacher, $mosque, $assignmentType, $assignment = null): ?array
    {
        ...
        return [
            ...
            if ($teacher->quranCircle) {
                ...
                $studentsData = $circle->students->map(function($student) {
                    return [
                        'id' => $student->id,
                        'الاسم' => $student->name,
                        'رقم_الهوية' => $student->identity_number,
                        ...
                        'عدد_الغياب' => $student->absence_count,
                ...
                $circlesData[] = [
                    ...
                    'المسجد' => [
                        'id' => $teacher->mosque->id ?? null,
                        'اسم' => $teacher->mosque->name ?? 'غير محدد'
                    ],
                    'إحصائيات' => [
                        'عدد_الطلاب' => $circle->students->count(),
                        'الطلاب_النشطون' => $circle->students->where('is_active', true)->count(),
                        'متوسط_الأجزاء' => $circle->students->avg('parts_count') ?? 0
                    ],
                    'الطلاب' => $studentsData
                ];
            }
            ...
            return response()->json([
                ...
                'البيانات' => [
                    ...
                    'الإحصائيات_العامة' => [
                        'عدد_الحلقات' => count($circlesData),
                        'إجمالي_الطلاب' => collect($circlesData)->sum('إحصائيات.عدد_الطلاب')
                    ],
                    'الحلقات' => $circlesData
                ]
            ], 200);
    ...
    public function getTeacherStudents($id): JsonResponse
    {
        try {
            ...
            $totalCircles = $allStudents->pluck('حلقة.id')->unique()->count();
            $totalMosques = $allStudents->pluck('مسجد.id')->unique()->count();
            
            // إحصائيات الحضور العامة
            $avgAttendanceRate = $allStudents->avg('حضور_الشهر_الحالي.نسبة_الحضور');
            $totalMemorizedPages = $allStudents->sum('منهج_حالي.عدد_الصفحات_المحفوظة');
            ...
            return response()->json([
                ...
                'بيانات_المعلم' => [
                    ...
                    'المسجد_الأساسي' => [
                        'id' => $teacher->mosque->id ?? null,
                        'الاسم' => $teacher->mosque->name ?? 'غير محدد',
                        'الحي' => $teacher->mosque->neighborhood ?? 'غير محدد'
                    ]
                ],
                'إحصائيات_عامة' => [
                    'عدد_الطلاب_الإجمالي' => $totalStudents,
                    'عدد_الطلاب_النشطين' => $activeStudents,
                    'عدد_الحلقات' => $totalCircles,
                    'عدد_المساجد' => $totalMosques,
                    'متوسط_نسبة_الحضور' => round($avgAttendanceRate, 1),
                    'إجمالي_الصفحات_المحفوظة' => $totalMemorizedPages,
                    'تاريخ_التحديث' => now()->format('Y-m-d H:i:s')
                ],
            ...
            return response()->json([
                'نجح' => false,
                'رسالة' => 'حدث خطأ في جلب بيانات طلاب المعلم',
                'خطأ' => $e->getMessage()
            ], 500);
        }
    }
    ...
    private function buildMosqueData($teacher, $mosque, $assignmentType, $assignment = null): ?array
    {
        ...
        $additionalCircles = \App\Models\TeacherCircleAssignment::where('teacher_id', $teacher->id)
            ...
            ->with(['circle.students' => function($q) {
                $q->with([
                    'quranCircle:id,name,grade_level,circle_type',
                    'circleGroup:id,name',
                    'curriculum:student_id,current_surah,current_ayah,memorized_pages',
        ...
        $circlesData = $circles->map(function($circle) use ($studentsData) {
            ...
            return [
                ...
                'عدد_الطلاب' => $circleStudents->count(),
                'الطلاب_النشطون' => $circleStudents->where('نشط', 'نعم')->count(),
                'متوسط_نسبة_الحضور' => $circleStudents->avg('حضور_الشهر_الحالي.نسبة_الحضور') ?? 0,
            ];
        })->toArray();
        ...
        return [
            ...
            'إحصائيات' => [
                ...
                'عدد_الطلاب' => $studentsData->count(),
                'الطلاب_النشطون' => $studentsData->where('نشط', 'نعم')->count(),
                'متوسط_نسبة_الحضور' => round($studentsData->avg('حضور_الشهر_الحالي.نسبة_الحضور'), 1),
                'إجمالي_الصفحات_المحفوظة' => $studentsData->sum('منهج_حالي.عدد_الصفحات_المحفوظة'),
            ],
