...
class TestCurriculumSystem extends Command
{
    protected $signature = 'test:curriculum-system 
    ...
    protected $description = 'اختبار شامل ومُصحح لنظام المناهج اليومية';    private $testMosque;
    private $testTeacher;
    private $testTeacherUser;
    private $testStudent;
    ...
    public function handle()
    {
        ...
        if ($this->option('full')) {
            $this->call('test:curriculum-system', ['--create-data' => true]);
            $this->call('test:curriculum-system', ['--test-api' => true]);
            $this->call('test:curriculum-system', ['--test-progression' => true]);
    ...
    private function createTestData()
    {
        ...
        try {            // إنشاء مسجد تجريبي
            ...
            ]);            // إنشاء مستخدم للمعلم التجريبي (مطلوب لجدول recitation_sessions)
            $this->testTeacherUser = User::create([
            ...
            $plans = [
                [
                    'curriculum_id' => $this->testCurriculum->id,
                    'name' => 'خطة الحفظ اليومية - الدرس الأول',
                    'plan_type' => 'الدرس',
                ...
                [
                    'curriculum_id' => $this->testCurriculum->id,
                    'name' => 'خطة المراجعة الصغرى - اليوم الأول',
                ...
                [
                    'curriculum_id' => $this->testCurriculum->id,
                    'name' => 'خطة المراجعة الكبرى - اليوم الأول',
                ...
                [
                    'curriculum_id' => $this->testCurriculum->id,
                    'name' => 'خطة الحفظ اليومية - الدرس الثاني',
                    'plan_type' => 'الدرس',
                ...
                [
                    'curriculum_id' => $this->testCurriculum->id,
                    'name' => 'خطة المراجعة الصغرى - اليوم الثاني',
                    'plan_type' => 'المراجعة الصغرى',
            ...
            $this->line('   👨‍🏫 المعلم: ' . $this->testTeacher->name . ' (' . $this->testTeacher->identity_number . ')');
            $this->line('   👨‍🎓 الطالب: ' . $this->testStudent->name . ' (' . $this->testStudent->identity_number . ')');
        ...
        try {
            ...
            $data = json_decode($response->getContent(), true);
              if ($data['success']) {
                ...
                if (isset($data['error'])) {
                    $this->error('تفاصيل الخطأ: ' . $data['error']);
                }
            }
        ...
        try {
            ...
            $this->info('📝 السيناريو الأول: إكمال الحفظ بدرجة عالية (8/10)');
              $request = new \Illuminate\Http\Request([
                'teacher_id' => $this->testTeacherUser->id,
                'recitation_type' => 'حفظ',
            ...
            if ($lowData['success']) {
                ...
                if (isset($lowData['error'])) {
                    $this->error('تفاصيل الخطأ: ' . $lowData['error']);
                }
            }
    ...
    }    private function loadTestData()
    {
        ...
        $this->testTeacherUser = User::where('email', 'test.teacher@garb.test')->first();
        
        if ($this->testMosque) {
            $this->testTeacher = Teacher::where('identity_number', '2234567890')->first();
            $this->testStudent = Student::where('identity_number', '3234567891')->first();
    ...
    {
        $this->info('🧹 تنظيف البيانات التجريبية...');        try {
                ...
                ->whereIn('student_curriculum_id', function($query) {
                    $query->select('id')
                          ...
                          ->whereIn('student_id', function($subQuery) {
                              $subQuery->select('id')
                                       ->from('students')
                                       ->where('identity_number', '3234567891');
                          });
                })
                ->delete();
            ...
            DB::table('curriculum_levels')
                ...
                ->delete();            DB::table('curricula')->where('name', 'منهج القرآن المُصحح')->delete();
            DB::table('teachers')->where('identity_number', '2234567890')->delete();
            DB::table('students')->where('identity_number', '3234567891')->delete();
            DB::table('users')->where('email', 'test.teacher@garb.test')->delete();
